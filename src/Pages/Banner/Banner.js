...
const Banner = () => {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are You Sure, You want delete?")) {
      deleteBannerMutationHandler(id);
    }
  };
  ...
  console.log({serviceBannerMutation})
  const deleteBannerMutationHandler = async (id) => {
    try {
      serviceBannerMutation.mutate(id, {
        ...
        return (
          <div className="flex items-center space-x-3">
            ...
            <Link to={"/banner/create/"}>
              <button onClick={()=>setEditBannerId(params.row.id)}
    ...
    <div className="p-3 md:p-5 ">
      <div className="bg-white p-2 md:p-5 rounded-lg">
        <div className="flex items-center justify-between py-4 ">
          ...
          <button onClick={()=>navigate('/banner/create')} 
          className="bg-blue-bg font-medium text-white p-2 px-5 rounded-md">+ Add</button>
        </div>
